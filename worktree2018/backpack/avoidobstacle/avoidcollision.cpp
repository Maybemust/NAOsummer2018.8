...
void AvoidCollision::start()
{
	...
	while (true)
	{
		...
		if ((direction == 1 && svm.rightdown_x() < leftboundry) || (direction == -1 && svm.leftdown_x() > rightboundry)) //&&(leftdown_y>judgepass))
			break;
	}
	sleep(5000);
	/*1-2之间直走*/
	...
	while (true)
	{
		...
		motionProxy.moveToward(0.1, direction * 0.5, 0);
		if (direction == -1 && svm.leftdown_x() > rightboundry) //&&(leftdown_y>judgepass))
		{
			break;
		}
		if (direction == 1 && svm.rightdown_x() < leftboundry) //&&(leftdown_y>judgepass))
		{
			break;
		}
	...
	while (true)
	{
		...
		svm.SVMFind(3, image);
		motionProxy.moveToward(0.6, 0, 0, config);
		if (svm.rightdown_y() - svm.rightup_y() > 140)
			break;
	}
	// waitKey(1000);
	sleep(1);
