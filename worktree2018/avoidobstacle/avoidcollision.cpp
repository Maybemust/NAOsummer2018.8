...
void AvoidCollision::start()
{
	...
	int leftboundry = 285; //while going right, if the obstacle is left enough
	int rightboundry = 60; //similar to above
	int numout=0;
	// waitKey(1000);
	wxzSVM svm;
	cout << "*************************" << "11111" << "*********************" << endl;
	...
	while (true)
	{
		...
		// 	motionProxy.moveToward(0.1, -0.8, 0);
		motionProxy.moveToward(0.5, direction * 0.7, 0);
	...
	//sleep(5000);
	/*1-2之间直走*/
	motionProxy.moveTo(0.0,0.0,-1*PI/15);
	cout << "*********************" << " 1-----2" << "**********************************" << endl;
	// waitKey(1000);
	while (true)
	{
	...
	// waitKey(1000);
	//sleep(1);
	//motionProxy.moveTo(0.0,0.0,-1*PI/8);
	cout << "*******************************" << "2222" << "*****************************************" << endl;
	/*绕过第二个柱子*/
	...
	while (true)
	{
		...
		motionProxy.moveToward(0.2, direction * 0.7, 0);
	...
	cout << "*********************" << " 2-----3" << "**********************************" << endl;
	// waitKey(1000);
	//motionProxy.angleInterpolation(names1, -1*PI/9, timeLists, isAbsolute);
	numout=0;
	while (true)
	{
		ALValue img = camProxy.getImageRemote(clientName);
...
//        	waitKey(50);
	...
	while (true)
	{
		...
		motionProxy.moveToward(0.4, direction * 0.7, 0);
		if ((direction == 1 && svm.leftdown_x() > leftboundry) || (direction == -1 && svm.rightdown_x() < rightboundry))
			break;
		cout<<svm.rightdown_x()<<" "<<svm.leftdown_x()<<endl;

	}
	//motionProxy.moveTo(1.0,0.8,-1*PI/10.0);
	while(true){
	motionProxy.moveToward(0.8, 0, 0, config);       
	}          //   能否准确到达 终点
