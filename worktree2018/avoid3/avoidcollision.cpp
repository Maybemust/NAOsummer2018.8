...
void AvoidCollision::start()
{
	...
	while (true)
	{
		...
		// 	motionProxy.moveToward(0.1, -0.8, 0);
		motionProxy.moveToward(0.6, direction * 0.5, 0);
	...
	while (true)
	{
		...
		motionProxy.moveToward(0.5, direction * 0.5, 0);
	...
	while (true)
	{
		...
		svm.SVMFind(3, image);
		motionProxy.moveToward(0.6, 0, 0, config);
		if (svm.rightdown_y() - svm.rightup_y() > 120)
			break;
		//in order to prevent robot from losing obstacle for too much time and that can be deleted
		if(svm.area()==0) 
			numout++;
...
//        	waitKey(50);
	...
	while(true){
	motionProxy.moveToward(0.7, 0, 0, config);       
	}          //   能否准确到达 终点
